...
/* Remark that if the Flags[OWN_IN_USABLE/SOS] is on, the Penguin which	*/
/* read the input (different Penguin from Whoami) allocated the clause to */
/* itself and did not count it in the initialization value of 		*/
/* Penguins[Whoami]. Therefore it is appropriate to increment it here.	*/
	}
...
/* of the two clauses belongs toa nother node.				*/
...
if (Flags[KNUTH_BENDIX].val && unit_clause(c) && a->varnum == NEG_EQ)
	return(KB_TARGET);
...
if (c->dest == ALL_PENGUINS)	/* non-input inference message */
	input_msg = 0;
else input_msg = 1;	/* input inference message */
